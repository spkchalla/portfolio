...
export function getAllContent(type) {
    ...
    const items = files.map(file => {
        ...
        if (!summary) {
            ...
            summary = paragraphs[0] ? (paragraphs[0].length > 160 ? paragraphs[0].slice(0, 160) + '...' : paragraphs[0]) : '';
        }
...
export function getAllSideQuests() {
    return getAllContent('side-quests');
}

/** Get all unfiltered thoughts */
export function getAllUnfiltered() {
    return getAllContent('unfiltered');
}
