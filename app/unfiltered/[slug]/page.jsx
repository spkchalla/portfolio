...
export async function generateStaticParams() {
    return getSlugsForType('unfiltered');
}

export async function generateMetadata({ params }) {
    try {
        const { slug } = await params;
        const { frontmatter } = getContentBySlug('unfiltered', slug);
...
export default async function UnfilteredPostPage({ params }) {
    let frontmatter, content;
    try {
        const { slug } = await params;
        ({ frontmatter, content } = getContentBySlug('unfiltered', slug));
    } catch {
        notFound();
    }
